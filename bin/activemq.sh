...
# a simple helper to get the activemq.sh installation dir
getActiveMQHome(){
  ...
  if [ -z "$REAL_DIR" ];then
      echo 'ERROR: unable to find real installtion path fo activemq.sh, you have to define ACTIVEMQ_HOME manually in the config' >&2
      exit 1
  fi
  echo "$REAL_DIR/"
...
if ( basename $0 | grep "activemq-instance-" > /dev/null);then
  INST="`basename $0|sed 's/^activemq.sh-instance-//;s/\.sh$//'`"
...
# load activemq.sh configuration
CONFIG_LOAD="no"
...
#
# @RET  : 0 => the activemq.sh process is running
#         1 => the activemq.sh process is not running
#
# Note: This function uses globally defined variables
# - $ACTIVEMQ_PIDFILE : the name of the pid file
...
#
# $1    : "checkforrunning", do not invoke the task if activemq.sh is not
#         active
